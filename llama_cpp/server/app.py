...
async def create_chat_completion(
    ...
    async with contextlib.asynccontextmanager(get_llama_proxy)() as llama_proxy:
        ...
        result = await run_in_threadpool(llama.create_chat_completion, **kwargs)
        ...
        audio_buffers = []
        for _, _, audio_file in generator:
            buffer = BytesIO()
            sf.write(buffer, audio_file, 24000, format='WAV')
            audio_buffers.append(buffer.getvalue())
